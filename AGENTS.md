...
config/secondmate-harness  optional model and effort pin for secondmate launches, on one line: "omp [<model>] [<effort>]" (section 4); LOCAL, gitignored; the harness token is always omp and only the model/effort tokens parametrize the secondmate. The primary's own setting; NOT inherited into secondmate homes (secondmates do not spawn secondmates)
config/backlog-backend  backlog backend override; LOCAL, gitignored; absent or "tasks-axi" = default tasks-axi backend, "manual" = force routine backlog updates to hand-editing; inherited by secondmate homes (section 10)
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then the herdr default; herdr is the first-class default backend (docs/herdr-backend.md) and tmux is a first-class alternative (docs/tmux-backend.md), while zellij, orca, and cmux are experimental spawn backends (docs/zellij-backend.md, docs/orca-backend.md, docs/cmux-backend.md) - herdr and cmux can also be selected by runtime auto-detection, zellij and orca never are (always explicit); not inherited into secondmate homes
...
`docs/configuration.md` owns the runtime-backend schema, `bin/fm-harness.sh` owns harness resolution (always `omp`), and `bin/fm-spawn.sh` owns launch flags and fail-closed validation.
The generic effort fallback and its precedence are owned by `harness-adapters`: explicit captain and standing configured effort win; otherwise use low for well-understood explicit work, xhigh for ambiguous investigation or design, intermediate levels proportionally, and never max without explicit captain preference.
Do not add model-specific versions of that policy.

`secondmate-provisioning` owns secondmate model/effort pins and inherited local material, while `harness-adapters` owns the harness consequences.
`config/secondmate-harness` may carry optional model and effort tokens (format `omp [<model>] [<effort>]`); the harness token is always omp.
...
Before dispatching a BATCH of two or more queued items, first triage whether any are already live or done, at THIS decision point rather than after spending crew and pool slots.
A "might already be satisfied" batch is a judgment call no bootstrap diagnostic can make for you, so it is steered here, not by a nudge line.
On omp, fan that read-only triage out to subagents (load `captain-parallel-digest`) and dispatch a crew only for the items that still need work.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section (or the wake digest) prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `FLEET_SYNC:`, `PR_CHECK_MIGRATION:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `NUDGE_SECONDMATES:`, `NUDGE_PARALLEL_DIGEST:`, or `FMX:`); silence and `BOOTSTRAP_INFO:` need no load.
