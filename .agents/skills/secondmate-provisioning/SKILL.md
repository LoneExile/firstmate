...
A bare `<harness>` (today's format, e.g. `omp`) behaves exactly as before - harness only, no model/effort flag - so this is fully backward-compatible.
...
Because this resolves from the file on every spawn, the pin is durable across every respawn (recovery, `/updatefirstmate`, restart) exactly like the harness axis itself - e.g. `config/secondmate-harness` containing `omp sonnet` keeps a secondmate pinned to that model even if the primary's own default model later changes.
...
The same launch and the same locked bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-harness`, `config/backlog-backend`, and the one shared captain-preference file `data/captain-shared.md`.
