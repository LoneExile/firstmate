...
secondmate_liveness_sweep() {
  ...
  # herdr path (fork R3): load one api session.snapshot per distinct session and
  # classify each meta from that cache (fm_backend_agent_state -> herdr
  # pane_agent_state -> _from_snapshot), falling back to per-endpoint probes on
  # snapshot failure, so a fleet of secondmates in one session costs one snapshot.
  # SECONDMATE_RESPAWNED_IDS tracking is intentionally omitted here: its only
  # reader is the #809 config-reread nudge, deliberately not ported in this fork.
  [ -d "$STATE" ] || return 0
  ...
  for meta in "$STATE"/*.meta; do
    ...
    if [ "$backend" = herdr ]; then
      ...
      # agent_state call (fm_backend_source is idempotent).
...
if [ -n "$tangle_branch" ]; then
  tangle_default=$(fm_expected_primary_branch "$FM_ROOT" 2>/dev/null || echo main)
  if [ "${FM_BOOTSTRAP_DETECT_ONLY:-0}" = 1 ]; then
