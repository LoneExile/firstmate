...
# glyph (e.g. a `| > ... |` bordered row). On a bare, unstructured row it is a
# dead-shell prompt and is NEVER "empty"; it classifies as `unknown` (not a safe
# injection target). The AGENT prompt glyphs `❯` and `›` are a genuine empty
...
# de-emphasized ghost text which a plain capture cannot tell apart from text a
# human typed, so the away-mode injector reads the idle pane as "pending input"
# and defers every escalation (the overnight wedge that motivated this
# consolidation). fm_composer_strip_ghost is the ONE ANSI-aware extractor of
# "real typed content": it drops every de-emphasized run - dim/faint (SGR 2) AND
...
# drift into per-harness one-off strips again.
#
...
#   - dim/faint runs (SGR 2): how some harnesses render ghost/suggestion text.
...
#     FM_COMPOSER_GHOST_LUMA_MAX (default 128): how some harnesses render
#     placeholder/hint text. A reset (SGR 0), a default-foreground (SGR 39), any
#     base foreground colour (30-37 / 90-97), or a lighter 38;2 foreground ends
#     the dark-foreground run. This assumes a DARK terminal theme, the firstmate
#     fleet reality, where real typed input is bright and only de-emphasised UI is
#     dark; the SGR-2 signal above stays theme-independent. A 256-colour foreground
#     (38;5;n) is NOT luminance-tested - it is palette-dependent and no fleet
#     harness uses it for ghost text, so it is kept (real text wins: under-stripping
#     merely defers, which the max-defer alarm surfaces, while over-stripping would
#     inject over real input).
...
#   [idle_re]  optional idle-placeholder regex (e.g. a harness's
