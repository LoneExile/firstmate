...
# so this exempts them while guarding every real secondmate home. The shared
# predicate lives in bin/fm-primary-scope-lib.sh so this guard and the
# session-start nudge (bin/fm-sessionstart-nudge.sh) cannot drift on primary
# detection.
