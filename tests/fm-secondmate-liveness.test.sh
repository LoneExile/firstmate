...
test_tmux_agent_state_classifies() {
  ...
  # omp fork: tmux has no positive-'alive' path. A live omp runs under a generic
  # bun/node interpreter that cannot be attributed from outside the pane
  # (docs/tmux-backend.md "Known gaps"), so every non-shell foreground - including
  # the removed-harness names upstream matched as alive - classifies as ambiguous,
  # which the sweep safely skips. Positive-alive lives only in the herdr snapshot
  # path (fm_backend_herdr_pane_agent_state), unit-tested separately below.
  for comm in bun claude codex opencode grok; do
    fb=$(make_probe_tmux "$TMP_ROOT/tmux-$comm" "$comm")
    out=$(PATH="$fb:$BASE_PATH" bash -c '. "$0/bin/fm-backend.sh"; fm_backend_agent_state tmux sess:win' "$ROOT")
    [ "$out" = ambiguous ] || fail "a non-shell $comm foreground process should classify as ambiguous for omp, got '$out'"
  done
  ...
  pass "fm_backend_tmux_agent_state: separates dead, missing, ambiguous, and unreadable (no positive-alive on tmux for omp)"
}
...
test_agent_state_dispatcher_and_compatibility() {
  ...
  # omp: tmux has no positive-alive path, so route with a bare shell -> dead,
  # which the tmux adapter classifies deterministically (proves dispatch to tmux).
  fb=$(make_probe_tmux "$TMP_ROOT/dispatch-tmux" zsh)
  out=$(PATH="$fb:$BASE_PATH" bash -c '. "$0/bin/fm-backend.sh"; fm_backend_agent_state tmux sess:win' "$ROOT")
  [ "$out" = dead ] || fail "detailed dispatcher should route tmux, got '$out'"
...
test_sweep_respawns_authoritatively_missing_secondmate() {
  local w fb tmuxfb log out
  ...
  assert_contains "$(cat "$log")" "new-window" "an authoritatively missing omp secondmate should be relaunched"
  assert_not_contains "$(cat "$log")" "kill-window" "an absent window should not need a destructive pre-kill"
  pass "sweep: an authoritatively missing omp secondmate window is relaunched"
}
