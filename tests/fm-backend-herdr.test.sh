...
# Real omp on Herdr renders no prompt glyph and no side border.
...
test_composer_state_omp_separator_idle_is_empty() {
  ...
  [ "$calls" -eq 1 ] || fail "omp separator recognition must corroborate identity exactly once, made $calls agent calls"
  pass "fm_backend_herdr_composer_state: a native idle omp separator composer reads empty"
}

test_composer_state_omp_separator_real_text_is_pending() {
  local dir log resp fb out
  dir="$TMP_ROOT/composer-omp-separated-pending"; mkdir -p "$dir/responses"; log="$dir/log"; resp="$dir/responses"; : > "$log"
  printf '\x1b[38;2;129;162;190m─────────────────────────────────────────────────────\x1b[0m\nprivacy safe human draft\x1b[7m \x1b[0m\n\x1b[38;2;129;162;190m─────────────────────────────────────────────────────\x1b[0m\n' > "$resp/1.out"
  printf '{"result":{"agent":{"agent":"omp","agent_status":"done"}}}\n' > "$resp/2.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_composer_state lab:w1:p2' "$ROOT" )
  [ "$out" = pending ] || fail "real text in a native omp separator composer should read pending, got '$out'"
  pass "fm_backend_herdr_composer_state: real omp composer text remains pending"
}

test_composer_state_omp_incomplete_separator_below_stale_generic_is_unknown() {
  local dir log resp fb out
  dir="$TMP_ROOT/composer-omp-separated-incomplete"; mkdir -p "$dir/responses"; log="$dir/log"; resp="$dir/responses"; : > "$log"
  printf '│   │\n─────────────────────────────────────────────────────\n\n' > "$resp/1.out"
  printf '{"result":{"agent":{"agent":"omp","agent_status":"idle"}}}\n' > "$resp/2.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_composer_state lab:w1:p2' "$ROOT" )
  [ "$out" = unknown ] || fail "an incomplete omp separator below a stale generic row should remain unknown, got '$out'"
  pass "fm_backend_herdr_composer_state: an incomplete lower omp separator cannot inherit a stale empty row"
}

test_composer_state_omp_separator_requires_safe_native_identity() {
  local dir log resp fb out status case_id idx=0
  for case_id in working non-omp unreadable over-tall; do
    dir="$TMP_ROOT/composer-omp-separated-$case_id"; mkdir -p "$dir/responses"; log="$dir/log"; resp="$dir/responses"; : > "$log"
    ...
    case "$case_id" in
      working) printf '{"result":{"agent":{"agent":"omp","agent_status":"working"}}}\n' > "$resp/2.out" ;;
      non-omp) printf '{"result":{"agent":{"agent":"shell","agent_status":"idle"}}}\n' > "$resp/2.out" ;;
      unreadable) printf '1\n' > "$resp/2.exit" ;;
      over-tall) printf '{"result":{"agent":{"agent":"omp","agent_status":"idle"}}}\n' > "$resp/2.out" ;;
    esac
    fb=$(make_herdr_fakebin "$dir")
    out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
      bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_composer_state lab:w1:p2' "$ROOT" )
    [ "$out" = unknown ] || fail "unsafe omp separator case '$case_id' must remain unknown, got '$out'"
  done
  pass "fm_backend_herdr_composer_state: omp separators never authorize working, non-omp, unreadable, or over-tall targets"
}
