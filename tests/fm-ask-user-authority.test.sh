...
test_primary_and_secondmate_instruction_generation() {
  ...
  # omp fork: the running-secondmate reread nudge lives in fm-bootstrap.sh's
  # SECOND_MATE_NUDGE_MESSAGE (the bootstrap sweep's instruction-surface advance),
  # not upstream's post-#809 SKILL.md CONFIG_REREAD wording (that config-reread
  # machinery is deliberately deferred in this fork - see UPSTREAM.md #809).
  assert_grep 're-read your AGENTS.md' "$BOOTSTRAP" \
