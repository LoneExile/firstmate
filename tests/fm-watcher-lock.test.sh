...
test_guard_warnings() {
  ...
  grep -F 'repair a missing or failed watcher cycle' "$err" >/dev/null || fail "guard banner missing the harness-aware fix command"
  grep -F 'queued wakes pending - drain them' "$err" >/dev/null || fail "guard did not warn about pending queue"
  grep -F 'After draining queued wakes, repair a missing or failed watcher cycle' "$err" >/dev/null || fail "guard did not order supervision repair after drain"
