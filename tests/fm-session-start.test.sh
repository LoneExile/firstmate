...
EOF
  ...
  printf '%s\n' omp > "$home/config/secondmate-harness"
...
EOF
  ...
  assert_grep 'harness=omp' "$home/state/$SESSION_START_SECOND_MATE_ID.meta" \
    "the real respawn path did not preserve the omp harness: $(cat "$home/state/$SESSION_START_SECOND_MATE_ID.meta")"
