...
REC=$(fm_transition_record "wG:pQ" "wG" "" "blocked" "omp")
...
[ "$(fm_transition_agent "$REC")" = "omp" ] || fail "agent accessor wrong: $REC"
...
[ "$(fm_transition_policy "done")" = "actionable" ] || fail "done must be actionable (terminal session-end edge)"
...
pass "fm_transition_policy is the single-owner status->action table (blocked/done=actionable, working=absorb, idle=defer, else=fallback)"
