...
When tasks are in flight and there is no live identity-matched watcher with a fresh beacon, the omp turn-end extension forces one bounded follow-up turn that tells the primary to resume the session-start supervision protocol.
...
Mechanism A, the autonomous wake, is a harness property: when a background watcher task exits, OMP re-invokes the model, which drains the wake, advances children, and re-arms a fresh watcher.
