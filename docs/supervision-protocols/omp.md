...
OMP (Oh My Pi) is a Pi fork; its extension API uses the `pi.*` namespace throughout. The watcher lifecycle follows the #397 fix: `stopArm()`, a one-shot `process.once("exit")` cleanup, awaited follow-up delivery, and the `fm_watch_arm_omp` tool + `/fm-watch-arm-omp` human fallback. Two key implementation details: the turn-end guard listens for `pi.on("turn_end")` because OMP has no `agent_settled` event, and the tool schema uses `pi.zod.object({})`. OMP sets `OMPCODE=1` (and `CLAUDECODE=1`) in its child/tool env, so `bin/fm-harness.sh` detects it as `omp` - and because `OMPCODE` is checked before `CLAUDECODE`, OMP never misdetects as the claude harness. OMP auto-loads `.omp/extensions/` only, never `.pi/`. VERIFIED live 2026-07-10 (omp v16.3.15): a real omp primary ran this loop end-to-end - the turn-end guard fired with two crewmate tasks in flight and the primary re-armed the watcher rather than ending blind, while two omp crewmates shipped green PRs from treehouse worktrees. (Not yet exercised: a seatbelt deny, the tmux-backend busy signature, and exit/interrupt keys.)
